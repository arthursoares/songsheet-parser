...
def _norm_text(t):
    """Lyric fragment comparison key: casefold, collapse whitespace."""
    return re.sub(r"\s+", " ", str(t or "")).strip().casefold()


def aligned_pairs(truth_names, cand_names):
    """Index pairs (ti, ci) of equal-name entries via difflib alignment."""
    sm = difflib.SequenceMatcher(a=truth_names, b=cand_names, autojunk=False)
    pairs = []
...
def score_song(truth_song, cand_song):
    ...
    chord_acc    aligned equal-name entries / truth entries
...
def score_corpus(song_pairs):
    ...
    """
    per_song = {}
    tot = {"pairs": 0, "t": 0, "v_ok": 0, "v_tot": 0, "x_ok": 0, "x_tot": 0, "a_ok": 0}
    ...
    for si in range(min(len(secs_a), len(secs_b))):
        ...
        for bi in range(min(len(bars_a), len(bars_b))):
            ...
            if names_a != names_b:
                ...
                continue  # voicing/text positions don't correspond once names differ
...
def cmd_score(args):
    ...
    print(f"{'song':<50} {'chord':>8} {'voicing':>8} {'text':>8} {'anchor':>8}  bars")
    for name, s in report["songs"].items():
        print(
            f"{name:<50} {_fmt_pct(s['chord_acc']):>8} {_fmt_pct(s['voicing_acc']):>8}"
    ...
    print(
        ...
        f" {_fmt_pct(a['chord_acc']):>8} {_fmt_pct(a['voicing_acc']):>8}"
